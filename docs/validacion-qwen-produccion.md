...
## Varias reservas y selección de profesional
Se separó el estado por sesión y especialidad. Pedir otro/nuevo turno inicia una reserva adicional conservando la anterior; una reprogramación modifica el último turno del profesional elegido. Las consultas de reserva existente y las correcciones se filtran por especialidad. Los nombres de los profesionales dichos en la conversación prevalecen sobre la selección anterior, y se conservan hasta que se cambie la selección o se nombre otro profesional. Los eventos de voz leen la selección actual desde una referencia de React, evitando capturar la selección inicial. Suite ampliada: 53 pruebas correctas y build de frontend correcto.

Verificación pública: una misma sesión conservó cinco reservas (dos en traumatología, una de control y dos en cardiología). Reprogramar y corregir el nombre de la reserva de control no alteró las otras cuatro. Se comprobó la elección del profesional tanto por selección de interfaz enviada en la petición como por nombre dentro del mensaje. El frontend publicado contiene la referencia actualizada para eventos de voz. Las cinco reservas de QA se archivaron y eliminaron por sesión.
