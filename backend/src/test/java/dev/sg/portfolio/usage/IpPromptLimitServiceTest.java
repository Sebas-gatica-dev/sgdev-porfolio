...
class IpPromptLimitServiceTest {
    ...
    void reportsStatusWithoutConsumingCredits() {
        ...
        assertThat(initial.newVisitor()).isTrue();
        assertThat(initial.used()).isZero();
        assertThat(afterStatusCheck.newVisitor()).isFalse();
    ...
    @Test
    void marksTokenRequestsAndAllowsAdminGrant() {
        IpPromptLimitService service = service(true, 2);
        String clientIp = "203.0.113.70";

        service.reservePrompt(clientIp);
        service.reservePrompt(clientIp);
        PromptLimitStatus exhausted = service.reservePrompt(clientIp);
        PromptLimitStatus requested = service.requestMoreTokens(clientIp);
        var row = service.adminRows().getFirst();

        assertThat(exhausted.allowed()).isFalse();
        assertThat(requested.tokenRequestPending()).isTrue();
        assertThat(row.clientIp()).isEqualTo(clientIp);
        assertThat(row.tokenRequestCount()).isEqualTo(1);

        var granted = service.grantTokens(new dev.sg.portfolio.domain.TokenGrantRequest(
                row.ipHash(),
                "",
                3,
                "MVP test"
        ));
        PromptLimitStatus afterGrant = service.status(clientIp);

        assertThat(granted.tokenLimit()).isEqualTo(5);
        assertThat(granted.tokensRemaining()).isEqualTo(3);
        assertThat(afterGrant.allowed()).isTrue();
        assertThat(afterGrant.tokenRequestPending()).isFalse();
    }
    ...
    private IpPromptLimitService service(JdbcTemplate jdbcTemplate, boolean enabled, int maxPrompts) {
        return new IpPromptLimitService(
                jdbcTemplate,
                new IpPromptLimitProperties(enabled, maxPrompts, 1, 5, 60, 300, "test-admin-token")
        );
    }
